<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.10" tiledversion="1.10.2" name="dungeon_prison_tileset" tilewidth="16" tileheight="16" tilecount="625" columns="25" objectalignment="topleft">
 <image source="../../Textures/Tilesets/dungeon_prison_tileset.png" trans="ff00ff" width="400" height="400"/>
 <tile id="234">
  <objectgroup draworder="index" id="2">
   <object id="1" x="0" y="0" width="16" height="16"/>
  </objectgroup>
 </tile>
 <wangsets>
  <wangset name="Dungeon Ground" type="corner" tile="30">
   <wangcolor name="Stone" color="#ff0000" tile="30" probability="1"/>
   <wangcolor name="StoneShadow" color="#00ff00" tile="33" probability="1"/>
   <wangtile tileid="4" wangid="0,2,0,1,0,2,0,2"/>
   <wangtile tileid="5" wangid="0,2,0,1,0,1,0,2"/>
   <wangtile tileid="6" wangid="0,2,0,2,0,1,0,2"/>
   <wangtile tileid="7" wangid="0,1,0,2,0,1,0,1"/>
   <wangtile tileid="9" wangid="0,1,0,1,0,2,0,1"/>
   <wangtile tileid="29" wangid="0,1,0,1,0,2,0,2"/>
   <wangtile tileid="30" wangid="0,1,0,1,0,1,0,1"/>
   <wangtile tileid="31" wangid="0,2,0,2,0,1,0,1"/>
   <wangtile tileid="33" wangid="0,2,0,2,0,2,0,2"/>
   <wangtile tileid="54" wangid="0,1,0,2,0,2,0,2"/>
   <wangtile tileid="55" wangid="0,1,0,2,0,2,0,1"/>
   <wangtile tileid="56" wangid="0,2,0,2,0,2,0,1"/>
   <wangtile tileid="57" wangid="0,2,0,1,0,1,0,1"/>
   <wangtile tileid="59" wangid="0,1,0,1,0,1,0,2"/>
  </wangset>
 </wangsets>
</tileset>
