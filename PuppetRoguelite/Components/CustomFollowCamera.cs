...
namespace PuppetRoguelite.Components
{
    public class CustomFollowCamera : Component, IUpdatable
    {
        ...
        public CustomFollowCamera(Entity targetEntity, Camera camera, CameraStyle cameraStyle = CameraStyle.LockOn, Measurement deadzoneMeasurement = Measurement.FixedPixel)
        {
        ...
        public CustomFollowCamera(Entity targetEntity, CameraStyle cameraStyle = CameraStyle.LockOn, Measurement deadzoneMeasurement = Measurement.FixedPixel)
            : this(targetEntity, null, cameraStyle)
        {
        }

        public CustomFollowCamera() : this(null, null)
        {
        }
        ...
        public virtual void Update()
        {
            ...
            Vector2 oldCameraPos = new Vector2(Camera.Position.X, Camera.Position.Y);
            Vector2 newTargetCameraPos = Vector2.Lerp(Camera.Position, Camera.Position + _desiredPositionDelta, FollowLerp);
            Vector2 cameraMovement = newTargetCameraPos - oldCameraPos;
            _movementRemainderX.Update(ref cameraMovement.X);// deal with subpixel movement, storing off any non-integar remainder for the next frame
            _movementRemainderY.Update(ref cameraMovement.Y);
        ...
        void OnGraphicsDeviceReset()
        {
            ...
            {
                var self = t.Context as CustomFollowCamera;
